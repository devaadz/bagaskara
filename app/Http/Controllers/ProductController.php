...
class ProductController extends Controller
{
    ...
    public function categoryStore(Request $request)
    {
        ...
        //deva
        logActivity('Membuat Category', "Pengguna membuat kategori: {$input['category_name']}");
        // logActivity('Cek Produk', 'cek produk makanan');
         // --- IGNORE ---
        $category_type = $request->category_type;
        $categories = Category::where('category_type', $request->category_type)->get();
        return view('admin.product.categoryChoose', compact('categories', 'category_type'));
    ...
    public function store(Request $request)
    {
        ...
        //deva
        logActivity('Membuat Produk', "Pengguna membuat produk: {$input['name']}");
        //end deva
         // --- IGNORE ---
        $stockData = [];
    ...
    public function destroy($id)
    {
        ...
        //deva
        logActivity('Menghapus Category', "Pengguna menghapus kategori: {$link->name}");
        //end deva
         // --- IGNORE ---
        Product::destroy($id);
        if ($link->type == 0) {
